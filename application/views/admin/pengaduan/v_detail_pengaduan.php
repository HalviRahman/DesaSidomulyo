  ...
  <div class="card">
    ...
    <div class="content">
      <div class="content table-responsive table-full-width">
        ...
        <button onclick="proses('<?= base_url("admin/$menu/proses/$detail->id_pengaduan/1") ?>')"
          class="btn btn-success btn-fill">Proses</button>
        <button onclick="proses('<?= base_url("admin/$menu/tolak/$detail->id_pengaduan") ?>')"
          class="btn btn-danger btn-fill">Tolak</button>
        <?php endif; ?>
        <?php } else { ?>
        <?php if ($_SESSION['role_admin'] == 1 || $_SESSION['role_admin'] == 2) : ?>
        <button onclick="buatTanggapan('<?= base_url("admin/pengaduan/komentar/$detail->id_pengaduan") ?>')"
          class="btn btn-info btn-fill">Beri Tanggapan</button>
        <?php endif; ?>
        <?php }
				if ($detail->status == 2) { ?>
        <?php if ($detail->kategori == 'anggaran' && ($_SESSION['role_admin'] == 1 || $_SESSION['role_admin'] == 2)) : ?>
        <button onclick="buatKegiatan('<?= base_url() ?>admin/kegiatan/form/tambah/<?= $detail->id_pengaduan ?>')"
          class="btn btn-success btn-fill">Buat Kegiatan</button>
        <?php endif; ?>
        <?php } ?>
      </div>
    </div>
